...
int *anotarse_1_svc(void *argp, struct svc_req *rqstp)
{
	static int result;
	if(jugando==-2){
		printf("Se ha registrado un nuevo jugador\n");
		result=id;
		clientes++;
		id++;
	}
...
struct tabla *resultadosparciales_1_svc(int *argp, struct svc_req *rqstp){
...
struct dados *jugar_1_svc(void *argp, struct svc_req *rqstp){
	...
	for (int i = 0; i < 5; ++i) {
		...
		Anterior.dados[i]=result.numeros[i];
	}
...
int *anotarresultados_1_svc(struct tabla *argp, struct svc_req *rqstp){
	...
	if (encontre) tablas[punteroSiguiente].puntaje[i] = argp->puntaje[i];
